...
{
    /// <summary>
    /// Rotates/maps angle easurements in a FIFO basis by 120 degress relative to the input and output measurement definitions in the connection string.
    /// </summary>
    /// <seealso cref="MeasurementMapping"/>
    /// <seealso cref="MeasurementSampler"/>
    /// <seealso cref="LinearStateEstimator"/>
    /// <seealso cref="SignalToNoiseRatio"/>
    /// <seealso cref="SnapshotManager"/>
    [Description("Angle Rotation : Rotates/maps angle measurements in a FIFO basis by 120 degress.")]
    public class AngleRotation : ActionAdapterBase
    {
        #region [ Private Members ]

        private MeasurementKey[] m_inputMeasurementKeys;
        private IMeasurement[] m_outputMeasurements;
        private double[] m_unrotatedMeasurements;
        ...
        /// <summary>
        /// The Initialize method for the AngleRotation Adapter
        /// </summary>
        public override void Initialize()
        {
            ...
            m_unrotatedMeasurements = new double[m_inputMeasurementKeys.Length];

            // Initialize the measurement values with zeros so they are never null;
            for (int i = 0; i < m_unrotatedMeasurements.Length; i++)
            {
                m_unrotatedMeasurements[i] = 0;
            }
        ...
        /// <summary>
        /// The method that is called when there are new measurements to be sent to the adapter.
        /// </summary>
        /// <param name="frame">The frame containing the new measurements.</param>
        ...
        protected override void PublishFrame(IFrame frame, int index)
        {
            ...
            // Create an empty List<> to store the rotated measurements
            List<IMeasurement> rotatedMeasurements = new List<IMeasurement>();

            // Use the input measurement keys to map measurements on a FIFO basis
            for (int i = 0; i < m_inputMeasurementKeys.Length; i++)
            {
                if (!inputMeasurements.TryGetValue(m_inputMeasurementKeys[i], out m_unrotatedMeasurements[i]))
                {
                    m_unrotatedMeasurements[i] = 9999;
                }
            }

            // Rotate the angle of the measurements by 120 degrees
            for (int i = 0; i < m_unrotatedMeasurements.Length; i++)
            {
                if (m_unrotatedMeasurements[i] <= 60)
                {
                    m_unrotatedMeasurements[i] = m_unrotatedMeasurements[i] + 120;
                }
                else
                {
                    m_unrotatedMeasurements[i] = m_unrotatedMeasurements[i] - 240;
                }
            }

            // If enough keys were specified to have a one-to-one mapping...
            if (m_outputMeasurements.Length == m_unrotatedMeasurements.Length)
            {
                // Prepare the measurements to be output
                for (int i = 0; i < m_outputMeasurements.Length; i++)
                {
                    rotatedMeasurements.Add(Measurement.Clone(m_outputMeasurements[i], m_unrotatedMeasurements[i], frame.Timestamp));
                }
            }
            else // Output all 9999
            {
                for (int i = 0; i < m_outputMeasurements.Length; i++)
                {
                    rotatedMeasurements.Add(Measurement.Clone(m_outputMeasurements[i], 9999, frame.Timestamp));
                }
            }

            // Output the rotated measurements
            OnNewMeasurements(rotatedMeasurements);
        }
