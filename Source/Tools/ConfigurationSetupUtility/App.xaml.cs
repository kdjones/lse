...
namespace ConfigurationSetupUtility
{
    ...
    public partial class App
    {
        ...
        public App()
        {
            ...
            // Attempt to create an event log source for the LinearStateEstimator Manager for authentication logging. This needs to be done
            // here since the CSU runs with administrative privileges and the LinearStateEstimator Manager normally does not; also there is
            // a short system delay that exists before you can write to a new event log source after it is first created.
            try
            {
                string applicationName = "LinearStateEstimator";

                // Create the event log source based on defined application name for LinearStateEstimator if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");

                applicationName = "LinearStateEstimator Manager";

                // Create the event log source based on defined application name for LinearStateEstimator Manager if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");
            }
            catch (Exception ex)
            {
                m_errorLogger.Log(new InvalidOperationException(string.Format("Warning: failed to create or validate the event log source for the LinearStateEstimator Manager: {0}", ex.Message), ex), false);
            }
        }
