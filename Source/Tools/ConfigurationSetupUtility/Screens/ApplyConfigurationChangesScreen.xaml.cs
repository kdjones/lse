...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ApplyConfigurationChangesScreen : IScreen
    {
        ...
        private void InitializeState()
        {
            ...
            m_LinearStateEstimatorManagerLocalCheckBox.IsEnabled = managerOptionsEnabled;
            m_LinearStateEstimatorManagerWebCheckBox.IsEnabled = webManagerOptionEnabled;

            // If the options are disabled, they must also be unchecked.
            if (!managerOptionsEnabled)
                m_LinearStateEstimatorManagerLocalCheckBox.IsChecked = false;

            if (!webManagerOptionEnabled)
                m_LinearStateEstimatorManagerWebCheckBox.IsChecked = false;

            m_setupHistorianCheckBox.IsChecked = false;

            // Set up the state object with the proper initial values.
            m_state["applyChangesToService"] = m_LinearStateEstimatorServiceCheckBox.IsChecked.GetValueOrDefault();
            m_state["applyChangesToLocalManager"] = m_LinearStateEstimatorManagerLocalCheckBox.IsChecked.GetValueOrDefault();
            m_state["applyChangesToWebManager"] = m_LinearStateEstimatorManagerWebCheckBox.IsChecked.GetValueOrDefault();
        ...
        // Occurs when the user chooses to apply changes to the LinearStateEstimator service.
        private void LinearStateEstimatorServiceCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = true;
        }

        // Occurs when the user chooses to not apply changes to the LinearStateEstimator service.
        private void LinearStateEstimatorServiceCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = false;
        }

        // Occurs when the user chooses to changes to the local LinearStateEstimator Manager application.
        private void LinearStateEstimatorManagerLocalCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the local LinearStateEstimator Manager application.
        private void LinearStateEstimatorManagerLocalCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = false;
        }

        // Occurs when the user chooses to apply changes to the LinearStateEstimator Manager web application.
        private void LinearStateEstimatorManagerWebCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToWebManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the LinearStateEstimator Manager web application.
        private void LinearStateEstimatorManagerWebCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
