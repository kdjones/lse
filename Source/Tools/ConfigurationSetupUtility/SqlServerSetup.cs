...
namespace ConfigurationSetupUtility
{
    ...
    public class SqlServerSetup
    {
        ...
        public void GrantDatabaseAccess(string loginName)
        {
            ...
            ExecuteStatement("IF DATABASE_PRINCIPAL_ID('LinearStateEstimatorAdminRole') IS NULL CREATE ROLE [LinearStateEstimatorAdminRole] AUTHORIZATION [dbo]");
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('{0}') IS NOT NULL AND DATABASE_PRINCIPAL_ID('LinearStateEstimatorAdminRole') IS NOT NULL EXEC sp_addrolemember N'LinearStateEstimatorAdminRole', N'{0}'", loginName));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('LinearStateEstimatorAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datareader', N'LinearStateEstimatorAdminRole'"));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('LinearStateEstimatorAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datawriter', N'LinearStateEstimatorAdminRole'"));
        }
                    ...
                    while ((object)line != null)
                    {
                        ...
                        else
                        {
                            if (trimLine.StartsWith("CREATE DATABASE") || trimLine.StartsWith("ALTER DATABASE") || trimLine.StartsWith("USE"))
                                line = line.Replace("LinearStateEstimator", DatabaseName);
