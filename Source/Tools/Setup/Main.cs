...
namespace Setup
{
    ...
    {
        // LinearStateEstimator product code, as defined in the setup packages
        ...
        private void buttonInstall_Click(object sender, EventArgs e)
        {
            ...
            RegistryKey LinearStateEstimatorInstallKey;

            LinearStateEstimatorInstallKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\" + ProductCode);

            // If key wasn't found, test for 32-bit virtualized location
            if (LinearStateEstimatorInstallKey == null)
                LinearStateEstimatorInstallKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\" + ProductCode);

            if (LinearStateEstimatorInstallKey != null)
            {
                if (MessageBox.Show("An existing version of the LinearStateEstimator is installed on this computer. Would you like to remove the existing version?\r\n\r\nCurrent configuration will be preserved.", "Previous Version Check", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                ...
                LinearStateEstimatorInstallKey.Close();
            }
        ...
        private bool RunSetup(SetupType type, bool closeOnSuccess)
        {
            ...
            // Install or uninstall LinearStateEstimator
            Process LinearStateEstimatorInstall = new Process();

            LinearStateEstimatorInstall.StartInfo.FileName = "msiexec.exe";
            ...
            if (type == SetupType.Uninstall)
            {
                ...
                // Uninstall any version of the LinearStateEstimator
                LinearStateEstimatorInstall.StartInfo.Arguments = "/x " + ProductCode + " /qr";
            }
            else
            {
                // Install current version of the LinearStateEstimator
                LinearStateEstimatorInstall.StartInfo.Arguments = "/i Installers\\LinearStateEstimatorSetup.msi";
            }

            LinearStateEstimatorInstall.StartInfo.UseShellExecute = false;
            LinearStateEstimatorInstall.StartInfo.CreateNoWindow = true;
            LinearStateEstimatorInstall.Start();
            LinearStateEstimatorInstall.WaitForExit();

            if (LinearStateEstimatorInstall.ExitCode == 0)
            {
                // Run configuration setup utility post installation of LinearStateEstimator, but not for uninstalls
                if (type == SetupType.Install)
                {
                    ...
                    installPath = AddPathSuffix(Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Grid Protection Alliance\LinearStateEstimator", "InstallPath", "").ToString().Trim());
                    targetBitSize = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Grid Protection Alliance\LinearStateEstimator", "TargetBitSize", "64bit").ToString().Trim();
                    ...
                    {
                        MessageBox.Show("Setup program was not able to launch the LinearStateEstimator Configuration Setup Utility due to an exception. You will need to run this program manually before starting the LinearStateEstimator.\r\n\r\nError: " + ex.Message, "Configuration Setup Utility Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
        ...
        private void AttemptToStopKeyProcesses()
        {
            ...
            ServiceController LinearStateEstimatorServiceController = null;

            try
            {
                foreach (ServiceController service in ServiceController.GetServices())
                {
                    if (string.Compare(service.ServiceName, "LinearStateEstimator", true) == 0)
                    {
                        LinearStateEstimatorServiceController = service;
                        break;
                    }
                }
            }
